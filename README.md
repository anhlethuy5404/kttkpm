...
1.  **Tải thư mục dữ liệu:**
    *   Tải thư mục `data/` từ liên kết tạm thời sau (hãy thay thế bằng link thực tế của bạn sau khi upload): 
        🔗 [Liên kết tải dữ liệu](https://drive.google.com/file/d/1M2-aYVt9aotjhl-Yn1LgMrKXDRnsnvjF/view?usp=sharing)
...
3.  **Khởi tạo Database & Nạp dữ liệu sản phẩm (Nếu dùng DB trống):**
    *Nếu bạn sử dụng file `db.sqlite3` có sẵn đi kèm dự án, bạn có thể bỏ qua bước này.*
    ...
    # Nạp sản phẩm phân cấp danh mục (giới hạn tối đa 50 sản phẩm/danh mục con để tối ưu hóa CPU) & tự động khởi tạo 2 tài khoản mặc định (admin/admin123 và user/user123)
...
Do PostgreSQL khởi động với CSDL trống, bạn cần nạp dữ liệu từ thư mục `data` vào PostgreSQL. Lệnh này sẽ giới hạn nạp **tối đa 50 sản phẩm cho mỗi danh mục con** (giúp tối ưu hóa tốc độ tính toán nhúng vector trên CPU) và tự động khởi tạo **2 tài khoản người dùng mặc định**:
*   **Tài khoản Admin (Superuser)**: Username: `admin` / Password: `admin123`
*   **Tài khoản User thường**: Username: `user` / Password: `user123`
